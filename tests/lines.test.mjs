...
const CALLS = [
  ...
  ["mcp__openovai__room", {}],
  ["mcp__openovai__recall", { store: "memory", query: "what the desk is" }],
...
describe("what a tool call is drawn as", () => {
  it("names each tool by what it does, from the call and never its result", () => {
    assert.deepEqual(CALLS.map(([name, input]) => line(name, input)), [
      ...
      null,
      "Looking around the room",
      "Recalling memory: what the desk is",
      "Recalling knowledge: k-12",
      "Recalling memory: all",
      "Remembering in knowledge: The suite runs without a browser.",
      "Writing the desk",
      null,
      null,
      null,
