...
describe("the rule Claude Code suggested for a call", () => {
  ...
  const carried = [
    ...
    ["a loop, whose whole command holds shell syntax", "Bash", { command: "for f in a b; do cmp x/$f $f; done" }, suggesting("Bash", "for f in a b; do cmp x/$f $f; done"), null],
    ["a pipe between two programs by path", "Bash", { command: "~/bin/a | ~/bin/b" }, suggesting("Bash", "~/bin/a | ~/bin/b"), null],
