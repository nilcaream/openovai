...
Two settings files are seeded once and never touched again, but for two things: an update wires
the hook named below into a `.claude/settings.json` that lacks it, and turns the harness's
attribution off in one that says nothing about it; it adds no rule, and says which of
...
the `git` rule that would let it through, whatever the hook says. And it turns the attribution
Claude Code would add to every commit and pull request off (`attribution`: an empty `commit` and
`pr`, `sessionUrl` false): a commit is the seat's, made as you asked, and says what its message
says — a `.claude/settings.json` that says something else about `attribution` is yours, and an
update leaves it. Every path rule starts with `/`, which anchors it at the instance root: a bare
