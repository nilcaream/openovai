...
// something", "for today" into a moment — the tool asks the helper (tools/helper.mjs). The helper
...
const FIELDS = ["id", "store", "kind", "source", "scope", "number", "until", "by", "at", "supersedes", "replaced", "reason"];
...
export function recordLine(record, { now, zone, successor }) {
  ...
  const replaced = record.supersedes === undefined ? "" : `; replaced ${record.supersedes} ("${record.replaced}")${record.reason === TAG ? "" : ` - ${record.reason}`}`;
  return `${first}\n      ${who}${replaced}`;
}
...
async function write(root, shape, { caller, now = new Date(), config, helper, zone = zoneOf() }) {
  ...
  // Step 10: the announcement, for a hard rule only.
