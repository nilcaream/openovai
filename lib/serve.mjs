...
// a park at the deadline, a plugin that could not be served. Every line of it is a row of
// lib/chat/log.mjs, this file's included.

import { parkRoom, settingsIn, stopping } from "./chat/lifecycle.mjs";
import { log, started } from "./chat/log.mjs";
...
async function main(argv) {
  ...
  if (aboutPlugins !== "") {
    log("plugins", null, null, aboutPlugins);
  }
  ...
  // The row `ovai start` waits for, and the one a person reads in the log to learn where the page
  // is. Whole, never "the port you installed with": with --port 0 nobody knows it until now. The
  // process is named here, once — its pid, host, user and version — and on no other row.
  started(root, `http://${HOST}:${server.address().port}`, version(root) ?? "unknown");
  ...
  // closed; (4) whoever is left is ended; (5) one row naming the signal, and exit. A server closed
  // first would refuse the very write_desk and stop_session calls the park waits for.
...
// The reason, as a row of the log like everything else the server says, and `ovai start` quotes
// it under its own sentence when the start fails.
main(process.argv.slice(2)).catch((error) => {
  log("error", null, null, error.message);
  process.exitCode = 1;
});
