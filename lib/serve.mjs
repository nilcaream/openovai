...
import { configFile, readConfig, readRoot } from "./instance.mjs";
...
async function main(argv) {
  ...
  if (wrongWindow !== null) {
    throw new Error(`${configFile(root)}: ${wrongWindow}`);
  }

  // And before anything is served, for the same reason: every session's permissions hang on one
  ...
  // process, so a directory read again later would show a new file while going on serving the old
