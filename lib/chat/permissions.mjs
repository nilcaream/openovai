...
export function shapeOf(request, root) {
  if (request.tool === "Bash") {
    ...
    const own = typeof command === "string" ? rulesForCommand(command, root) : null;
    return own ?? (typeof command === "string" && SUBSTITUTION.test(command) ? null : suggested(request, root));
  }
...
//
// Nor is a whole command a button when nobody would keep it as a rule: an exact command longer
// than a line of the card, or one holding shell syntax, is a one-off written out in full, and it
// fills the panel with a rule that never matches again. Allow and Deny answer it; the words on a
// button are the rule it writes, so it is left off rather than shortened. Nor is one file: the
// checker accepts it for a rule settled by hand, but a button granting the file it was pressed for
// and nothing near it would stop the next call like this one all the same.
//
// Nor is anything Claude Code suggests for a command holding a backtick or a `$(`: it reads the
// span as a command of its own and suggests a prefix from inside it — measured, a `grep` whose
// pattern held a backtick was offered `Bash(Use the:*)`, words from the pattern that name no
// program. Checking each prefix against the leading words of the command would drop the right
// suggestion for a later side of a compound too, so the command itself is what is passed over.
const SUBSTITUTION = /`|\$\(/;
const EXACT_AT_MOST = 80;
const SHELL_SYNTAX = /[\n|&;<>$`]/;
function oneOff(rule) {
  const exact = /^Bash\((.*)\)$/s.exec(rule);
  return exact !== null && !rule.endsWith(":*)") && (exact[1].length > EXACT_AT_MOST || SHELL_SYNTAX.test(exact[1]));
}
...
function suggested(request, root) {
  ...
  for (const suggestion of Array.isArray(request.suggestions) ? request.suggestions : []) {
    ...
    for (const { toolName, ruleContent } of suggestion.rules) {
      ...
      if (accepted !== null && !oneOff(accepted) && !FILE_RULE.test(accepted) && !rules.includes(accepted)) {
        rules.push(accepted);
      }
    }
  }
...
// so no `..`, no `./`; `Edit(/**)` and `Read(/**)` are the root. `Edit(/dir/file)`,
// `Read(/dir/file)`: one file inside the instance, the same anchor and no star — measured on
// 2.1.280, an ask or a deny on one file stops that file and nothing beside it, and the narrower ask
// is consulted over a wider allow, which is what the seeded ask on the common note relies on. A
// button never carries one (see `suggested`). `Read(//dir/**)`: a subtree
...
export function acceptRule(rule, root) {
  ...
  const named = (SUBTREE_RULE.exec(rule) ?? FILE_RULE.exec(rule))?.[2];
  return named !== undefined && inside(named, root) === named ? rule : null;
}
