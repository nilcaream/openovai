...
// once the call failed, with the reason the result gave for a tooltip. `names.chat` is who the
...
// reads `<User> → <Worker>` on a ground of its own, since it is not a prompt to the Leader.
//
...
// went, `Writing` in red with the reason when it did not; its own answer stays its reply.
export function row(entry, names) {
  ...
  if (entry.from === "user") {
    ...
    return { who: names.user, kind: "user", text: String(entry.text) };
  }
