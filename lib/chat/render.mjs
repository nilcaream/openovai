...
// line finds it. The three carry `placeholder` too: true when the markdown opens with something
// other than a paragraph, so the page folds the row behind a placeholder rather than a first line
// it does not have.
  ...
  if (typeof entry.to === "string") {
    ...
    if (names.seat === names.leader) {
      if (entry.from === names.seat) {
        ...
        return { who: `To ${entry.to}`, kind: "peer-out", ...folded(entry.text), ...status, ...msg };
      }
