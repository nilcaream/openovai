...
// <addressee>` on a ground of its own — not a prompt to the Leader, and not the User's words, though
// shown as they are, like the User's. It carries the id, so a click on either Worker's line finds it.
export function row(entry, names) {
  ...
  if (typeof entry.to === "string") {
    ...
    if (entry.overheard === true) {
      return { who: `${entry.from} → ${entry.to}`, kind: "overheard", text: String(entry.text), ...msg };
    }
