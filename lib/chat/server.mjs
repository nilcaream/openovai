...
export function toolsFor(instance, caller) {
  ...
  return [
    ...
    // a session that can read why it would be refused does not go hunting for another way round.
    {
      name: "recall",
      description:
        "Read what this workspace knows. The store is managed by a model, not by you: a write without replaces may have replaced a record its writer never named, and what you read is what stands after that — it is not a file, and recall and remember are not create, read, update and delete over a MEMORY.md you know from elsewhere. store: memory or knowledge. Exactly one of query (words, matched by meaning by a model, one request on the account), id (one record) or all (every current record); kind narrows to hard-rule, fact or trap, and kind hard-rule alone gives the numbered set with its version; expired true adds records whose until has passed, marked. Each record comes as written, with what it replaced: that record's id and text, whether the writer named it or the store chose it, and the reason. A replaced record is reachable by id only, and nobody is told when a fact or trap is replaced — read again rather than trust. Refused, in this order: two of query, id and all, or none; kind hard-rule with store knowledge; a query while the model is held for quota, does not answer or answers badly (ask again). A rule scoped to the Leader is not in a Worker's answer.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "remember",
      description:
        "Write one record of what this workspace knows. The store is managed by a model, not by you: it is not a file, and remember and recall are not create, read, update and delete over a MEMORY.md you know from elsewhere. store: memory or knowledge. kind: hard-rule, fact or trap (knowledge has no hard rules). text: the record, stored as written; a hard rule is one line. replaces: the id of the current record this one supersedes, and reason says why. until: when it runs out — an ISO 8601 moment with an offset, words such as 'for today', or now to retire. source user: the User said it. scope leader: keeps a User's hard rule from Workers. Without replaces a model judges whether the text is one current record of the same store and kind restated, widened, narrowed or reversed, and the record it judges so is replaced, on the store's judgement alone; there is no way to say the text is new, and only the answer tells you what happened. Replacing a record that replaced the wrong one does not bring the wrong one back: a replaced record stays replaced, reachable by its id through recall and by nothing else — to bring it back, write it again. The answer names what was replaced, quotes its text, says whether you named it or the store chose it, and gives the reason. Refused, in this order: kind hard-rule, source user or scope leader from a Worker; scope leader on anything but a User's hard rule; a kind the store has not got; a hard rule with a newline or over the length cap; an until that cannot be placed or has passed; replaces naming a record that is missing, of another store or kind, or already replaced; the model held for quota, not answering or answering badly when the store chooses (nothing is written; ask again); a team write over the User's record, the Leader included, whether you named it or the store chose it; a hard rule over the count cap.",
      inputSchema: {
        type: "object",
        properties: {
          store: { type: "string", enum: ["memory", "knowledge"], description: "memory or knowledge." },
