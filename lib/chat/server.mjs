...
import { DeskError, LEDGER, LISTS, allowAsked, archiveFor, deskFile, hire as openDesk, isModel, isName, modelFor, nextName, retire as closeDesk, ruleAsked, headerFields, writeDeskHeader } from "../desks.mjs";
import { LEADER, WORKER, TURN_PATIENCE, end, interrupt, isSeat, recordOf, resume, running, seats, waiting } from "./session.mjs";
import { aboutSeat, arm, deliver, heldLine, isParking, parkRoom, showRules, startSeat } from "./lifecycle.mjs";
...
function showTheEnding(root, seat, answered) {
  answered.then((reply) => {
    if (reply.ended === true) {
      append(root, seat, { from: SERVER, text: `${seat} stopped before answering: ${reply.text}`, failed: true });
...
// After a stop: how many frames wait and which goes in next — the User's own line, a message
// from a seat, or an event of the chat's — with the time it was told, so a seat that is busy
// again at once is seen to be busy with something older than the press.
function stoppedLine({ count, next }) {
  if (next === null) {
  ...
  return `stopped; ${count} waiting, next: ${named} (${quota.hhmm(next.at)})`;
}
...
//
// The User's line goes ahead of what waits on the seat — messages, events — on any panel they
// type on: the User steers, and a seat with a backlog of messages would otherwise work through
// every one of them before reading a word the User typed. The trade-off: a message a seat was
// sent earlier is answered after a line the User typed later.
function typed(instance, seat, text) {
  ...
  const told = deliver(instance, seat, userFrame(text), { ahead: true, written: () => delivered(root, seat, index) });
...
export function toolsFor(instance, caller) {
  ...
  const root = instance.root;
  // The store's two tools, with what the store is handed about who is calling: the seat and its
  // role, as the server knows them; the store applies every rule on its own side. Both go through
  // a model when a judgement is needed — a recall by meaning, a remember with no `replaces` named,
  // an `until` in words — and that round-trip is what makes them slow, so it is timed here, where
  // the seat is known: one log line per round-trip, and one saying so when none was needed.
  const clock = instance.clock ?? Date.now;
  const viaStore = async (name, args, call) => {
    if (!offered(name)) {
      return { refused: `${name} is not offered to you` };
    }
    let modelled = false;
    const helper = async (question, request) => {
      ...
      const began = clock();
      try {
        return await askTheHelper(instance, question, request);
      } finally {
        log("store", caller.seat, null, `${name} model in ${clock() - began} ms`);
      }
    };
    const answered = await call(root, args, { caller: { name: caller.seat, role: caller.role }, now: new Date(), config: instance.config, helper });
    if (!modelled) {
      log("store", caller.seat, null, `${name} no model`);
    }
    return answered;
  };
  ...
  return [
    ...
    // a session that can read why it would be refused does not go hunting for another way round.
    {
      name: "recall",
      description:
        "Read what this workspace knows. The store is managed by a model, not by you: it is not a file, and recall and remember are not create, read, update and delete over a MEMORY.md you know from elsewhere. store: memory or knowledge. Exactly one of query (words, matched by meaning by a model, one request on the account), id (one record) or all (every current record); kind narrows to hard-rule, fact or trap, and kind hard-rule alone gives the numbered set with its version; expired true adds records whose until has passed, marked. Each record comes as written, with what it replaced: that record's id and text, and the writer's reason — every replacement was named by its writer, the store replaces nothing on its own. A record brought back by restore says who restored it, when, and what had replaced it, which still stands. A replaced record is reachable by id only, and nobody is told when a fact or trap is replaced or restored — read again rather than trust. Refused, in this order: two of query, id and all, or none; kind hard-rule with store knowledge; a query while the model is held for quota, does not answer or answers badly (ask again). A rule scoped to the Leader is not in a Worker's answer.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      run: (args) => viaStore("recall", args, recall),
    },
    {
      name: "remember",
      description:
        "Write one record of what this workspace knows, or bring a replaced one back. The store is managed by a model, not by you: it is not a file, and remember and recall are not create, read, update and delete over a MEMORY.md you know from elsewhere. store: memory or knowledge. kind: hard-rule, fact or trap (knowledge has no hard rules). text: the record, stored as written; a hard rule is one line. replaces: the id of the current record this one supersedes, and reason says why; or none, when you have read the store and this is a new record. until: when it runs out — an ISO 8601 moment with an offset, words such as 'for today', or now to retire. source user: the User said it. scope leader: keeps a User's hard rule from Workers. Without replaces a model judges whether the text is one current record of the same store and kind restated, widened, narrowed or reversed; when it is, nothing is written and the refusal names that record — its id, its text, the model's reason — and you re-issue with replaces naming it if you agree, or replaces none if you do not. The store never replaces on its own judgement, and a write never lands unseen. restore: the id of a replaced record, alone with store, brings it back live under its own id, as it was; the record that replaced it stands — a restore is not a swap — and nobody is told. Writes to one store run one at a time. The answer is the record's line: what it replaced, quoted, and the reason; for a restore, the record brought back, who restored it, and what had replaced it. Refused, in this order: kind hard-rule, source user or scope leader from a Worker; scope leader on anything but a User's hard rule; a kind the store has not got; text missing; reason without replaces naming a record; a hard rule with a newline or over the length cap; an until that cannot be placed or has passed; replaces naming a record that is missing, of another store or kind, or already replaced; without replaces, a text that reads as a current record restated, widened, narrowed or reversed (the refusal names it), or the model held for quota, not answering or answering badly (ask again); a team write over the User's record, the Leader included; a hard rule over the count cap. A restore is refused with anything beside store and restore; naming what is not a record you can see; naming a current record; naming a hard rule (write it again with replaces naming the rule that holds its number); naming the User's record from a Worker.",
      inputSchema: {
        type: "object",
        properties: {
          store: { type: "string", enum: ["memory", "knowledge"], description: "memory or knowledge." },
          ...
          replaces: { type: "string", description: "The id of the current record this one replaces, or none for a record that replaces nothing. Left out, a model looks for a record this one restates, and finding one refuses the write naming it." },
          reason: { type: "string", description: "Why it replaces what it replaces. Goes with replaces naming a record." },
          restore: { type: "string", description: "The id of a replaced record to bring back live, as it was. Alone with store; the record that replaced it stands." },
        },
        required: ["store"],
        additionalProperties: false,
      },
      offered: offered("remember"),
      run: (args) => viaStore("remember", args, remember),
    },
    {
      name: "write_desk",
      description:
        "Write the header of your own desk, desks/<you>/STATE.md: title (one line, up to 120 characters — what you are on) and status (one line, up to 80 characters, no |). The header is line 1 and the server's; everything below it is yours, edited in place with the file tools like any other file, and this call leaves it exactly as it stands. Edit the body first, then call this: it is what says the desk is current, and restart_session and stop_session refuse until it was called since the event that asked, or since this turn. Refused: an empty title or status, a newline in either, a | in status, any other key.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      run(args) {
        ...
        const record = recordOf(caller.seat);
        const written = writeDeskHeader(root, caller.seat, record === undefined ? fields : { ...fields, rules: record.rules });
        if (written === null) {
        ...
        return { text: `desk header written: desks/${caller.seat}/STATE.md (title and status; the body as it stands; rules ${written.rules})` };
      },
    },
    {
    ...
    // (postPermission below) and reaches the Leader as an event. The tool never writes a rule.
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          rule: { type: "string", description: "The rule, as Claude Code's reference spells it: Bash(word:*), Bash(the whole command), Edit(/dir/**), Read(/dir/**), Read(//dir/**), WebFetch(domain:host), WebSearch, mcp__server__tool, Agent(Name), or a bare tool name." },
      ...
      run(args) {
        ...
        if (accepted === null) {
          return { refused: `${rule} is not a rule Claude Code reads: Bash(word:*) or Bash(the whole command), Edit(/dir/**) or Read(/dir/**) under the instance root, Read(//dir/**) outside it, WebFetch(domain:host), WebSearch, mcp__server__tool, Agent(Name), or a bare tool name` };
        }
    ...
    async run(args) {
      ...
      try {
        ...
        if (ms >= SLOW_SAID_FROM) {
          append(instance.root, seat, { from: SERVER, text: `${tool.name} took ${ms} ms` });
        }
      }
    },
  }));
