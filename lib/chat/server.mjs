...
// of the chat's once a call took ten seconds or more, the same threshold and shape as the wait on
// a card. A call under that draws nothing: the row explains a gap, and there is none.
export const SLOW_SAID_FROM = 10;
  ...
  return tools.map((tool) => ({
    ...
    async run(args) {
      ...
      try {
        ...
        if (seconds >= SLOW_SAID_FROM) {
          append(instance.root, seat, { from: THE_CHAT, text: `${tool.name} took ${seconds} s` });
        }
      }
    },
  }));
