...
import { THE_CHAT, append, delivered, failedAsSaid, read } from "./conversation.mjs";
...
function stoppedLine({ count, next }) {
  ...
  const named =
    ...
    : `${next.event} event from the chat`;
    ...
    async run(args) {
      ...
      try {
        ...
        if (ms >= SLOW_SAID_FROM) {
          append(instance.root, seat, { from: THE_CHAT, text: `${tool.name} took ${ms} ms` });
        }
      }
    },
  }));
...
async function handle(instance, port, request, response) {
  ...
  if (onSession !== null) {
    ...
    if (request.method === "POST" && what === "stop") {
      ...
      if (interrupted) {
        append(instance.root, seat, { from: THE_CHAT, text: stoppedLine(waiting(seat)) });
        resume(seat);
      } else {
        console.log(`no turn to stop: ${seat}`);
        append(instance.root, seat, { from: THE_CHAT, text: "no turn to stop" });
      }
