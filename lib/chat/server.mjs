...
import { DeskError, LEADER, LEDGER, LISTS, WORKER, allowAsked, archiveFor, deskFile, hire as openDesk, isModel, isName, modelFile, modelFor, nextName, retire as closeDesk, ruleAsked, headerFields, writeDeskHeader, writeModel } from "../desks.mjs";
...
import { localAt, references } from "./refs.mjs";
...
// bearer: the four modules that decide what is drawn (a dialog, the panels, a row, a reference), the markdown
...
const ASSETS = Object.freeze({
  ...
  "/refs.mjs": { file: path.join(HERE, "refs.mjs"), type: "text/javascript; charset=utf-8" },
...
function typed(instance, seat, text) {
  ...
  // Every reference in the line that names a row on this panel goes in after it, the whole row,
  // resolved against the rows as they stood before the line. Who wrote it is a name a session
  // knows: a row stores the seat, "Server", or "user" for the User's own, which is the User's name.
  const rows = read(root, seat);
  const refs = references(rows, rows.length, text)
    .filter(({ index }) => index !== null)
    .map(({ to, index }) => ({
      to,
      from: rows[index].from === "user" ? instance.config.user : rows[index].from,
      at: localAt(rows[index].at),
      text: String(rows[index].text ?? ""),
    }));
  const index = append(root, seat, { from: "user", text });
  const told = deliver(instance, seat, userFrame(text, refs), { written: () => delivered(root, seat, index) });
  ...
  if (seat !== instance.config.leader) {
    ...
    // The references go with it, resolved against the Worker's rows: the Leader's panel has none of them.
    const woken = deliver(instance, leader, serverEvent("user-typed", { who: seat }, text, refs));
  ...
  return [
    ...
    },
    {
      ...
      run(args) {
        ...
        // The model the desk named before this hire's word replaced it — null when it named none —
        // put back when the start fails, so a hire that never ran leaves the desk as it found it.
        let kept;
        if (!hadDesk) {
          try {
          ...
          kept = fs.existsSync(modelFile(root, name)) ? fs.readFileSync(modelFile(root, name), "utf8") : null;
        ...
        try {
          ...
          if (kept === null) {
            fs.rmSync(modelFile(root, name), { force: true });
          } else if (kept !== undefined) {
            fs.writeFileSync(modelFile(root, name), kept);
          }
          throw error;
        }
