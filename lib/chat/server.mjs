...
function typed(instance, seat, text) {
  ...
  if (told.refused !== undefined) {
    append(root, seat, { from: SERVER, text: `${seat} has no process`, failed: true });
  } else {
...
// a slow call and a stuck seat can be told apart while watching: the log says `tool: <seat>
// <name> in N ms` — `failed in N ms: <why>` when the tool threw — and the seat's panel gets a row
  ...
  return tools.map((tool) => ({
    ...
    async run(args) {
      ...
      try {
        ...
        console.log(failed === null ? `tool: ${seat} ${tool.name} in ${ms} ms` : `tool: ${seat} ${tool.name} failed in ${ms} ms: ${failed.message}`);
  ...
  if (onSession !== null) {
    ...
    if (request.method === "POST" && what === "stop") {
      ...
      if (interrupted) {
        ...
        console.log(`no turn to stop: ${seat}`);
