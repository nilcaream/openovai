...
// What the server process says, from the moment `ovai start` starts it. Every run appends under a
// header line of its own, so the account of a run that was replaced survives the start that
// replaced it — which is when somebody wants to read it.
