...
// happens for an index the conversation has not got.
...
// results in what it said too, and that row stays as it landed.
export function failedAsSaid(root, session, text) {
  const messages = read(root, session);
  const index = messages.findLastIndex((entry) => entry.from === session);
  if (index === -1 || messages[index].text !== text || messages[index].failed !== undefined) {
    return null;
  }
