...
export const EVENTS = Object.freeze(["user-typed", "overheard", "context-full", "restarted", "quota-low", "idle", "stopped", "park", "hard-rules", "permission"]);
...
// rule: a server event, a hard-rules update, takes its place among the messages and the User's
// lines; one frame alone is a queue of one. Composed from built frames only — their text is
// already neutralised, and the envelope's own tag name is neutralised in every body — so nothing
// inside can close the envelope; and never from an envelope, since a queue holds turns and not
// queues. `items` is `{ frame, at }` pairs in arrival order, `at` a moment in ms.
