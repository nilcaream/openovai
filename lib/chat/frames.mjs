...
// before it is framed: any open or close form of our five tag names — the four frames, and the
// `<ref>` a User's frame carries — inside it has its `<` turned into `&lt;`, and nothing else about
// it changes. The frame's own tags are then the only ones in
...
const FRAME_TAG = /<(\s*\/?\s*)(user|message|server-event|queue|ref)(?![A-Za-z0-9_])/gi;
...
class Frame {
  constructor(key, kind, text, event = null, from = null, urgent = false) {
    if (key !== BUILT_HERE) {
    ...
    // reading the text back; and whether the Leader sent it urgent, so it joins a turn under way.
    this.from = from;
    this.urgent = urgent;
...
// The User's words exactly as typed, and after them one `<ref>` the server wrote for every
// reference in them that names a row: `refs` is `{ to, from, at, text }` each, `to` the
// reference's `HH:MM:SS/mmm`, `from` the name of who wrote the row, `at` its whole time and
// `text` all of it.
function withRefs(words, refs, close) {
  if (refs.length === 0) return `${neutralise(words)}${close}`;
  const children = refs.map(({ to, from, at, text: body }) => `\n    <ref${attributes({ to, from, at })}>${neutralise(body)}</ref>`);
  return `${neutralise(words)}${children.join("")}\n  ${close}`;
}

export function userFrame(text, refs = []) {
  return new Frame(BUILT_HERE, "user", `<user>${withRefs(text, refs, "</user>")}`);
}

// An urgent message says so to the one who reads it: it came into the turn under way.
export function messageFrame(from, text, { urgent = false } = {}) {
  const marked = urgent === true ? ` urgent="true"` : "";
  return new Frame(BUILT_HERE, "message", `<message from="${token("from", from)}"${marked}>${neutralise(text)}</message>`, null, from, urgent === true);
}

// Every kind of event the server says. A kind not in here is a bug upstream, refused here.
export const EVENTS = Object.freeze(["user-typed", "overheard", "context", "restarted", "quota-low", "quota-reset", "idle", "stopped", "park", "permission", "admin-closed", "checkpoint", "undelivered", "died", "closing", "done"]);

// `refs` are the `<ref>` children a user-typed event carries after the words, as userFrame's.
export function serverEvent(kind, attrs = {}, body = undefined, refs = []) {
  ...
  return new Frame(
    BUILT_HERE,
    "server-event",
    body === undefined ? `${opening}/>` : `${opening}>${withRefs(body, refs, "</server-event>")}`,
    { type: kind, attrs },
  );
}
