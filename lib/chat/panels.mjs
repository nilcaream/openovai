...
// The line's tooltip: when each window resets, on the page's own clock, each from its own moment —
// "5h resets today at 12:12, 7d on Monday at 13:41, 7d fable tomorrow at 23:11". A window with no
// reset is left out.
const WEEKDAYS = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];

function resetSaid(iso, now) {
  const at = new Date(iso);
  const tomorrow = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1);
  const sameDay = (a, b) => a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();
  const day = sameDay(at, now) ? "today" : sameDay(at, tomorrow) ? "tomorrow" : `on ${WEEKDAYS[at.getDay()]}`;
  return `${day} at ${String(at.getHours()).padStart(2, "0")}:${String(at.getMinutes()).padStart(2, "0")}`;
}

export function quotaTitle(quota, now) {
  if (quota === null) return "";
  const given = Object.entries(quota.resets).filter(([, iso]) => iso !== null);
  return given.map(([window, iso], index) => `${window}${index === 0 ? " resets" : ""} ${resetSaid(iso, now)}`).join(", ");
}
...
export function delivery(delivered, busy, name, waited = null) {
  if (delivered) {
    if (waited !== null && waited.seconds >= LATE_AFTER) {
      return { text: `delivered ${waited.clock} ✓ — ${waited.seconds} seconds after`, glyph: DELIVERED_GLYPH, wait: false };
    }
...
// A click on a row's stamp puts the reference to it (refs.mjs) into the box where the cursor is,
// in place of what is selected: a space before it unless it starts the box or follows one, and a
// space after it unless one follows already. Answers the box's new value and where the cursor goes,
// just after the reference and its space.
export function insertRef(value, start, end, ref) {
  const before = value.slice(0, start);
  const after = value.slice(end);
  const lead = before === "" || /\s$/.test(before) ? "" : " ";
  const trail = /^\s/.test(after) ? "" : " ";
  const put = `${lead}${ref}${trail}`;
  return { value: `${before}${put}${after}`, caret: before.length + put.length + (trail === "" ? 1 : 0) };
}
