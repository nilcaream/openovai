...
// How the page stands to the server: on its stream, told the server is stopping, or without a
// stream — the server gone, or not yet reached again.
...
function panelOf(state, about, now) {
  return {
    name: about.name,
    model: about.model ?? "",
    effort: typeof about.effort === "string" ? about.effort : "",
...
function seen(state, about, now, { fromSnapshot }) {
  ...
  panel.effort = typeof about.effort === "string" ? about.effort : panel.effort;
...
// a turn runs, waiting for you while it asks. A dimmed panel gets no word, and neither does any
// panel while the page has no stream: the red dot is the mark, and the server that would know
// is not there to ask.
...
// The head of a panel, in parts: the name, the facts (the model with its effort when one is set,
// the context in k when known) and the state word.
export function head(state, name) {
  const panel = state.panels[name];
  const model = panel.effort === "" ? panel.model : `${panel.model}/${panel.effort}`;
  const facts = [model, panel.context === null ? "" : `${Math.round(panel.context / 1000)}k`];
