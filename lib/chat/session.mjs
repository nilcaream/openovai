...
// every answer belongs to the write before it, and a seat that was busy reads everything that
// came while it was, together and in order, rather than one turn at a time.
...
export function start(instance, seat, { asked = nobodyToAsk, ended = nothing, turned = nothing, queue = [], gate = () => null, held = nothing, changed = nothing, called = nothing, returned = nothing, failed = nothing, said = nothing } = {}) {
  ...
  const record = {
    ...
    held,
    changed,
    // The rule-set version this process was told, at spawn and after every update written.
...
// leaves the queue in order, each turn keeping its arrival, and what it passes goes in. A
// hard-rules update is a frame like any other, and once it is written the desk header says which
// set this process has now been told.
//
...
function drain(record) {
  ...
  record.child.stdin.write(`${JSON.stringify({ type: "user", message: { role: "user", content: record.turn.frame.text } })}\n`);
  log("wrote", record.seat, null, `${describe(record.turn)}, ${record.queue.length} waiting`);
  record.turn.written();
  if (record.turn.rules !== null) {
    record.rules = record.turn.rules;
...
// and the last to arrive. What it asks of the seat is what the last to arrive asks: an ask with
// anything after it is a reprieve. `rules` is the set the newest hard-rules update in it says,
// or null.
function queueOf(turns) {
  return {
    ...
    ask: turns.at(-1).ask,
    at: turns[0].at,
    rules: turns.filter((turn) => turn.frame.event?.type === "hard-rules").map((turn) => turn.frame.event.attrs.set).at(-1) ?? null,
