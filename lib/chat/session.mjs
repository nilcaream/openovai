...
// `why` being the first line the result said (an exit code, a file that is not there).
export function start(instance, seat, { asked = nobodyToAsk, ended = nothing, turned = nothing, queue = [], gate = () => null, held = nothing, changed = nothing, called = nothing, failed = nothing } = {}) {
  ...
  child.stdout.on("data", (chunk) => {
    record.rest = frames(String(chunk), record.rest, (frame) => {
      ...
      // doing. The result of a call comes back as the user's turn of the protocol, and what is
      // read off it is whether the call failed and, then, the first line of what it said.
