...
import { desks, modelFor, persona, personaFile, readDesk, writeDeskHeader } from "../desks.mjs";
...
// its turn and the helper runs a whole Claude Code run of its own; Claude Code's own default
// would give up on either.
...
// of its makes — and `failed` with `{ id, why }` once a call's result comes back as an error,
// `why` being the first line the result said (an exit code, a file that is not there). `said` is
...
export function start(instance, seat, { asked = nobodyToAsk, ended = nothing, turned = nothing, queue = [], gate = () => null, held = nothing, changed = nothing, called = nothing, failed = nothing, said = nothing } = {}) {
  ...
  child.stdout.on("data", (chunk) => {
    record.rest = frames(String(chunk), record.rest, (frame) => {
      ...
      // it is whether the call failed and, then, the first line of what it said.
...
function drain(record) {
  ...
  if (record.turn !== null) {
    console.log(`not written: ${record.seat} turn open since ${new Date(record.turnBegan).toISOString()}, ${record.queue.length} waiting`);
    return;
  }
  if (record.ending !== null) {
    console.log(`not written: ${record.seat} ending=${record.ending}, ${record.queue.length} waiting`);
    return;
  }
  const held = [];
  ...
  console.log(`wrote: ${record.seat} ${record.turn.frame.kind} #${record.turn.order}, ${record.queue.length} waiting`);
  record.turn.written();
  if (prefix.length > 0) {
...
export async function interrupt(seat, { patience = INTERRUPT_PATIENCE, thenDrain = true } = {}) {
  ...
  console.log(`interrupt: ${seat} written, turn ${turn.frame.kind} #${turn.order}`);
  ...
  console.log(heard ? `interrupt: ${seat} result in ${Date.now() - written} ms` : `interrupt: ${seat} no result after ${patience} ms — seat freed`);
  if (record.turn === turn) {
    record.turn = null;
    turn.resolve({ interrupted: true, text: "interrupted" });
    record.changed(record);
...
// What waits for a seat: how many frames, and the one that goes in next — its kind, who it is
// from when it is a message, its event type when it is one, and when it was told — or null when
// nothing waits. For saying, after a stop, what the seat is about to be busy with.
export function waiting(seat) {
  const queue = processes.get(seat)?.queue ?? [];
  const next = queue[0];
  return {
    count: queue.length,
    next: next === undefined ? null : { kind: next.frame.kind, from: next.frame.from, event: next.frame.event?.type ?? null, at: next.at },
  };
}
...
// Put one frame on one seat's queue. Told `ahead`, it goes in front of everything waiting and
// behind what was told ahead before it — so what the User types keeps its own order and still
// beats every message and event queued before it.
//
...
// byte is refused.
export function tell(seat, frame, { ahead = false, ask = null, written = nothing } = {}) {
  if (!isFrame(frame)) {
    throw new Error("a session is told frames built by frames.mjs, and nothing else");
  }
  const record = processes.get(seat);
  ...
  const answered = new Promise((resolve) => {
    const turn = { frame, resolve, ahead, ask, written, order: arrival(), at: record.clock() };
    order = turn.order;
    if (ahead) {
      record.queue.splice(record.queue.findLastIndex((one) => one.ahead) + 1, 0, turn);
    } else {
      record.queue.push(turn);
    }
  });
  console.log(`queued: ${seat} ${frame.kind} #${order}${ahead ? " ahead" : ""}, ${record.queue.length} waiting`);
  drain(record);
