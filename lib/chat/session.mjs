...
// What one request took of the context: its input, cached and fresh. Null when nothing was said.
function inputOf(request) {
  if (request === undefined || request === null) {
    return null;
  }
  return (request.input_tokens ?? 0) + (request.cache_read_input_tokens ?? 0) + (request.cache_creation_input_tokens ?? 0);
}
...
// `returned` with `{ id }` once a call's result comes back, whatever it says. `died` is called
// with `{ text }` when a seat that has answered before has a turn come back an error, once for
// each run of such turns; a seat that never answered is ended instead. `said` is
...
// session says is on its panel as it says it, and not only once its turn is over. `measured` is
// called with the record whenever a request of a turn under way puts the context at a new size.
export function start(instance, seat, { asked = nobodyToAsk, ended = nothing, turned = nothing, queue = [], spoke = false, gate = () => null, held = nothing, typing = () => ({ quiet: 0, cap: 0 }), changed = nothing, called = nothing, returned = nothing, failed = nothing, died = nothing, said = nothing, measured = nothing } = {}) {
  ...
  const record = {
    ...
    held,
    changed,
    // The typing hold: its settings, read at every drain; when a key last went into this seat's
    // box on the page; since when a turn waits for it, and the timer that looks again.
    typing,
    typedAt: null,
    heldSince: null,
    holdTimer: null,
    ...
    askedAt: null,
    askedWhy: null,
    // How long a close may wait for its desk, in ms from the turn that read it; null for none.
    closeDeadline: null,
    turnBegan: null,
    ending: null,
    // Whether `end` was asked to take this process down: one that goes without it went on its own.
    endAsked: false,
    // Whether this seat has ever answered a turn, carried from a predecessor across a restart
    // the way the queue is. It is what tells a hire that never worked from a seat that stopped
    // working later: the first turn after a hire is the test of the hire.
    spoke,
    // What the service said when the turn that ends this seat died. Kept because the process is
    // gone by the time anybody is told, and its stderr says nothing about a turn.
    diedOf: null,
    // Whether the run of dead turns this seat is in has been told; cleared by an answered turn.
    deathTold: false,
    doing: null,
    lastCall: null,
    ...
    open: new Set(),
    // The `system/init` frames seen, and what last joined a turn under way, until the next write.
    inits: 0,
    joinedLast: null,
    idleSince: now,
    idleTold: 0,
  ...
  child.stdout.on("data", (chunk) => {
    record.rest = frames(String(chunk), record.rest, (frame) => {
      ...
      // Every turn opens with one `system/init`, whoever began it (measured on 2.1.280). One that
      // finds no turn open, other than the process's first, is a turn the run began itself.
      if (frame.type === "system" && frame.subtype === "init") {
        record.inits += 1;
        if (record.inits > 1 && record.turn === null) {
          selfStarted(record);
        }
      }
      ...
      if (frame.type === "assistant") {
        if ((frame.parent_tool_use_id ?? null) === null) {
          // Every request's frames carry its usage as the turn runs (measured on 2.1.280), so the
          // context is known at each request and not only at the turn's result. A turn the service
          // refused says a request of none, which is not a size.
          const context = inputOf(frame.message?.usage);
          if (context !== null && context > 0 && context !== record.context) {
      ...
      if (frame.type === "result" && record.turn !== null) {
        ...
        // What the turn came to, read before anything is reset. A turn the page stopped is the
        // seat's own ending and not a failure of the service's, so it is not a death here.
        const answer = answerIn(frame, said_);
        const dead = answer.failed && record.interrupting === null;
        record.turn = null;
        record.said = 0;
        record.open.clear();
        if (!dead) {
        ...
        if (dead) {
          // The service's own words, verbatim: the one time this happened they named the fix,
          // and nothing worked out here could have.
          log("died", seat, null, answer.text);
        }
        turn.resolve(answer);
        if (dead && !record.spoke) {
          // A seat that has never answered and whose turn died cannot work, and cannot ask to
          // stop either: the stop a session asks for wants a desk written since its turn began,
          // and a seat whose every turn dies has never had a turn it could write one in.
          record.ending = "first-turn-failed";
          record.diedOf = answer.text;
          end(seat);
          return;
        }
        // A process somebody is already ending (stdin closed) is not completing a close: its last
        // turn may come back failed on the way out, and that is not the close's doing.
        if (record.ending === null && !record.child.stdin.writableEnded && closedBy(record, dead)) {
          record.ending = closedAs(record.askedWhy);
          end(seat, TURN_PATIENCE);
          // What waits is not written now, and the log says so; a restart carries it.
          drain(record);
          return;
        }
        if (dead && !record.deathTold) {
          // A seat that has worked may be over a failure that passes, so it lives, and whoever
          // started it is told once for a run of deaths: the next answered turn ends the run.
          record.deathTold = true;
          died({ text: answer.text });
        }
        turned(record);
        drain(record);
      } else if (frame.type === "result") {
        // A result with no turn open and no init before it. Not expected; said, and nothing else.
        log("unpaired", seat, null, "a result with no turn open");
        const context = contextOf(frame.usage);
        if (context !== null) {
          record.context = context;
        }
  ...
  const gone = (why, exit) => {
    ...
    // whoever started this one spawns; any other ending answers them ended, and unread, since no
    // turn ever held them. The turn that was running is answered ended either way: its process is
    // gone, and what it said on stderr, if anything, goes with the answer.
    const running = record.turn;
    ...
    for (const turn of left) {
      turn.resolve(turn === running ? { ended: true, text: why, err: record.err.trim() } : { ended: true, unread: true, text: why });
    }
    ...
    // `own`: nobody ended it — no ending of its own, and `end` never asked; it crashed or was
    // killed from outside.
    ended({ seat, role, ending: record.ending, own: record.ending === null && record.endAsked !== true, exit, carried, why, context: record.context, spoke: record.spoke });
  };
...
function drain(record) {
  ...
  takeAsk(record, record.turn.ask, record.turnBegan);
  writeFrame(record, record.turn.frame);
  record.joinedLast = null;
  log("wrote", record.seat, null, `${describe(record.turn)}, ${record.queue.length} waiting${released(record, passing)}`);
  record.turn.written();
...
// next call, or for the turn's end. A message the Leader sent urgent goes in the same way. The
// queue goes in up to and including its last frame that joins, in arrival order, as one queue
// frame; the turn grows by it and its one `result` answers them all. A queue with no such frame
// in it waits for the turn's end, as ever. A close never joins: it is read at the start of a turn
// of its own, so a seat is never asked to close in the middle of a step, and whatever arrived
// after it waits with it. Answers whether it wrote.
function joined(record) {
  if (record.open.size === 0 || record.ending !== null || record.interrupting !== null) {
    return false;
  }
  const closing = record.queue.findIndex((turn) => isClosing(turn.ask));
  const last = (closing === -1 ? record.queue : record.queue.slice(0, closing)).findLastIndex((turn) => joins(turn.frame));
  const held = [];
  const passing = [];
  ...
  record.turn.order ??= joining.order;
  record.turn.last = joining.last;
  record.joinedLast = { ...joining, at: record.clock() };
  takeAsk(record, joining.ask, record.clock());
  writeFrame(record, joining.frame);
  log("wrote", record.seat, null, `${describe(joining)} into the turn open since ${new Date(record.turnBegan).toISOString()}, ${record.queue.length} waiting${released(record, passing)}`);
  joining.written();
  record.changed(record);
  return true;
}

// What joins a turn under way: the User's words, and a message the Leader sent urgent.
function joins(frame) {
  return frame.kind === "user" || frame.urgent === true;
}

// The User typing on this seat's panel holds a turn with none of the User's words in it: while a
// key went into the box within the last `quiet` seconds, and for no more than `cap` seconds in
// all, what waits is not written, so what the User is about to send goes in with it rather than
// behind a turn the seat set off on meanwhile. Only a turn about to begin is held — a run is not
// paused between its own calls — and a User frame is never held: it takes the queue in with it.
// A `quiet` or `cap` of 0 turns the hold off. A timer looks again when the quiet or the cap runs
// out; every keystroke that comes meanwhile moves the quiet on and never the cap. Answers whether
// it held.
function typingHolds(record) {
  const { quiet, cap } = record.typing();
  if (quiet <= 0 || cap <= 0 || record.typedAt === null || record.queue.some((turn) => turn.frame.kind === "user")) {
    return false;
  }
  const now = record.clock();
  const quietEnds = record.typedAt + quiet * 1000;
  const capEnds = (record.heldSince ?? now) + cap * 1000;
  if (now >= quietEnds || now >= capEnds) {
    return false;
  }
  if (record.heldSince === null) {
    record.heldSince = now;
    log("held", record.seat, null, `typing, ${record.queue.length} waiting`);
  }
  clearTimeout(record.holdTimer);
  record.holdTimer = setTimeout(() => {
    record.holdTimer = null;
    drain(record);
  }, Math.min(quietEnds, capEnds) - now);
  return true;
}

// A hold is over once what waited is written: its timer goes, and the `wrote` row says for how
// long it held and what ended it — the quiet, the cap, or the User's own line. Answers that, or
// nothing when there was no hold.
function released(record, written) {
  clearTimeout(record.holdTimer);
  record.holdTimer = null;
  if (record.heldSince === null) {
    return "";
  ...
  const now = record.clock();
  const held = ((now - record.heldSince) / 1000).toFixed(1);
  const ended = written.some((turn) => turn.frame.kind === "user") ? "user" : now - record.heldSince >= record.typing().cap * 1000 ? "cap" : "quiet";
  record.heldSince = null;
  return `, after typing held ${held}s (${ended})`;
}

// A turn the run began itself — for a line that reached it after the last ask of the turn it was
// written into (measured on 2.1.280). The seat is busy with it like any other: a turn owned by
// nobody, which what waits stays behind and a User frame may join, and whose result answers only
// what joined it. What joined the turn before it was answered by that turn and is not answered
// again: for the User's frames the answer draws only the ending row, and the words this turn says
// reach the panel as it says them. The log names them.
// A known gap, unmeasured: a turn the server writes right after a result while the run is already
// beginning one of its own. Its init then finds the server's turn open, the run's result answers
// it, and the result after that finds no turn and is logged as unpaired.
function selfStarted(record) {
  const after = record.joinedLast === null ? "nothing joined" : `after ${describe(record.joinedLast)} joined at ${new Date(record.joinedLast.at).toISOString()}`;
  log("self-started", record.seat, null, `a turn the run began itself, ${after}`);
  const turns = [];
  record.turn = {
    frame: null,
    turns,
...
// A close is the server ending a seat on somebody's word: the ask `close:<why>`, why being stop,
// restart, idle, quota or park. The seat writes its desk in the turn that reads it, and the close
// completes when that turn is over (`closedBy`). One that holds is not taken back by a message or
// the User's line after it — only another close replaces it; a close for idling is a reprieve like
// any other ask, since a word to an idle seat is exactly what it was waiting for.
export function isClosing(ask) {
  return typeof ask === "string" && ask.startsWith("close:");
}

function holds(ask) {
  return isClosing(ask) && ask !== "close:idle";
}

// The ending a close comes to: the reason it was asked for.
export function closedAs(ask) {
  return ask.slice("close:".length);
}

// What a turn asks is taken as the seat's ask, unless the turn asks nothing either way or a close
// that holds is pending and this is not a close.
function takeAsk(record, ask, at) {
  if (ask === undefined || (holds(record.askedWhy) && !isClosing(ask))) {
    return;
  }
  record.askedWhy = ask;
  record.askedAt = ask === null ? null : at;
}

// A close completes at the end of the turn that read it, or of any later turn, once the desk was
// written since it was asked — never at the write itself, so everything the seat does after its
// desk in that turn is done before it goes. A turn that died completes it too, the desk as it is:
// a seat whose turns die has no turn left to write one in.
function closedBy(record, dead) {
  return isClosing(record.askedWhy) && (dead || (record.deskWrittenAt !== null && record.deskWrittenAt >= record.askedAt));
}
...
// A close that holds is the exception: it is what the turn asks whatever came after it.
function queueOf(turns) {
  const speaking = turns.filter((turn) => turn.ask !== null || turn.frame.kind !== "server-event").at(-1);
  const holding = turns.filter((turn) => holds(turn.ask)).at(-1);
  return {
    frame: queueFrame(turns.map((turn) => ({ frame: turn.frame, at: turn.at }))),
    turns,
    order: turns[0].order,
    last: turns.at(-1).order,
    ask: holding !== undefined ? holding.ask : speaking === undefined ? undefined : speaking.ask,
    at: turns[0].at,
    resolve: (reply) => turns.forEach((turn) => turn.resolve(reply)),
    written: () => turns.forEach((turn) => turn.written()),
  };
...
function describe(turn) {
  if (turn.turns.length === 0) {
    return "a turn the run began itself";
  }
...
// A key went into the box on this seat's panel: what the typing hold hangs off. A seat with no
// process has nothing to hold.
export function keystroke(seat) {
  const record = processes.get(seat);
  if (record !== undefined) {
    record.typedAt = record.clock();
  }
}
...
// before it could answer, and `unread: true` besides when it closed before the frame was written —
// or `{ refused: "no process" }` when the seat has none, in which case
