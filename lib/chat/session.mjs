...
export function start(instance, seat, { asked = nobodyToAsk, ended = nothing, turned = nothing, queue = [], gate = () => null, held = nothing, changed = nothing, called = nothing, returned = nothing, failed = nothing, said = nothing } = {}) {
  ...
  const gone = (why, exit) => {
    ...
    ended({ seat, role, ending: record.ending, exit, carried, why });
  };

  child.on("error", (error) => {
    gone(error.code === "ENOENT" ? "Claude Code is not on the PATH of the process serving this page" : error.message, error.code ?? error.message);
  });
...
function drain(record) {
  ...
  log("wrote", record.seat, null, `${record.turn.frame.kind} #${record.turn.order}, ${record.queue.length} waiting`);
  record.turn.written();
  if (prefix.length > 0) {
...
export async function interrupt(seat, { patience = INTERRUPT_PATIENCE, thenDrain = true } = {}) {
  ...
  log("interrupt", seat, null, `written, turn ${turn.frame.kind} #${turn.order}`);
...
export function tell(seat, frame, { ahead = false, ask = null, written = nothing } = {}) {
  ...
  log("queued", seat, null, `${frame.kind} #${order}${ahead ? " ahead" : ""}, ${record.queue.length} waiting`);
  drain(record);
