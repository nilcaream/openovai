...
// of its makes — `returned` with `{ id }` once a call's result comes back, whatever it says, and
// `failed` with `{ id, why }` once a call's result comes back as an error, `why` being the first
// line the result said (an exit code, a file that is not there). `said` is
