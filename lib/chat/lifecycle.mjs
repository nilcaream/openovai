...
import { LEADER, WORKER, arrival, end, interrupt, recordOf, roleOf, running, runningSeats, seats, start, tell } from "./session.mjs";
...
// and once, with whatever is queued, when the last request took more of the context than the
// ceiling allows.
...
export function arm(instance) {
  ...
  // The hard-rules update is a frame like any other: told to every running seat the set reaches,
  // it takes its place in the seat's queue at its arrival and goes through the gate as anything
  // does.
  const offRules = onHardRulesChanged((version, renderFor) => {
    for (const seat of runningSeats()) {
      const text = renderFor(recordOf(seat).role);
      if (text !== null) {
        deliver(instance, seat, rulesUpdateFrame(version, text));
      }
    }
  });
