...
function afterClose(instance, { seat, role, ending, carried, context = null, why = "", spoke = false }) {
  seatChanged(instance, seat);
  if (ending !== "restart") {
  ...
  if (ending === "restart") {
    ...
    if (holding === null) {
      ...
      // A Worker restarts only on the Leader's order now, and the order's outcome is this.
      if (role === WORKER) {
        deliver(instance, leaderOf(instance), serverEvent("stopped", { who: seat, why: "restart" }));
      }
      return;
    }
  ...
  for (const seat of runningSeats()) {
    ...
    if (minutes >= idle.force) {
      ...
      if (seat === leader) {
        ...
        close(instance, seat, "idle", { deadline: IDLE_GRACE * 60 });
      }
    }
  }
}
