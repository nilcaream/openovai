...
function asking(instance, seat) {
  return (request) => {
    ...
    // The one call the Leader's panel draws: the one that stopped on a card. A Worker's was drawn
    // when it was made; the Leader's is drawn here, where the card is, since the User is being
    // asked about it and the card alone would not say which call it is.
...
export function startSeat(instance, seat, { queue = [] } = {}) {
  const started = start(instance, seat, {
    ...
    // are not — its panel is the User's conversation — but for the one that stops on a card,
    // drawn by asking(), and the one it is at, said by the seat while the turn runs (doingBy).
...
export function arm(instance) {
  ...
  const offStage = quota.onStage((window, stage, resets, model) => {
    stageReached(instance, window, stage, resets, model).catch((error) => log.log("quota", null, null, `stage ${stage} on ${window}: ${error.message}`));
  });
