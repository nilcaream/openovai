...
// stood still for minutes would otherwise read as a run that took that long.
function asking(instance, seat) {
  return (request) => {
    ...
    waiting.then((decision) => {
      ...
      append(instance.root, seat, { from: THE_CHAT, text: `waited ${seconds} s for permission: ${what}` });
    });
...
export function deliver(instance, seat, frame, { ahead = false, ask = null, written = () => {} } = {}) {
  ...
  if (!running(seat)) {
    ...
    if (holding !== null) {
      ...
      holdTurn(seat, { frame, ahead, ask, resolve, written, order: arrival() }, holding);
