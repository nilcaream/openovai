...
// config key -> { resetsAt (ms), model } of a window that reached a stage, until its reset is
// told. Apart from `announced`, which only a later reading clears: while every session is
// stopped no reading comes, and the reset still has to be seen.
...
function announce(now) {
  ...
  for (const [window, { key, model = null }] of Object.entries(WINDOWS)) {
    ...
    pendingReset.set(key, { resetsAt: reading.resetsAt, model });
...
// Every window that reached a stage and whose reset has passed since, as { window (config key),
// model or null }, taken off the list: each reset is answered once. The clock decides, as it does
// for a reading; no new reading is needed.
export function resetsPassed(now = clock()) {
  const passed = [];
  for (const [key, { resetsAt, model }] of pendingReset) {
    if (resetsAt <= now) {
      pendingReset.delete(key);
      passed.push({ window: key, model });
    }
  }
  return passed;
}

// Every reset still to come forgotten, so none is answered: for a room that parks.
export function forgetResets() {
  pendingReset.clear();
}
...
// Whether a process may be started on this model — a spawn. The same criterion.
