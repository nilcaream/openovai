...
import { CUSTOMIZATION_ASK_RULES, DESK_TEMPLATE, OWN_ACCOUNT_RULES, READ_RULES, SHELL_DENY_RULES, SHELL_RULES, STALE_DENY_RULES, TOOL_RULES, TREE_RULES, deskFile, deskRules, readTemplate, render } from "./desks.mjs";
...
export function rulesMissing(root) {
  ...
  return [
    ...
    ...CUSTOMIZATION_ASK_RULES.filter((rule) => !ask.includes(rule)).map((rule) => `ask ${rule}`),
  ];
}
