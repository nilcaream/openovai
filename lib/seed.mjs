...
import { DESK_DENY_RULES, DESK_TEMPLATE, OWN_ACCOUNT_RULES, READ_RULES, SHELL_DENY_RULES, SHELL_RULES, TOOL_RULES, TREE_RULES, deskFile, deskRules, readTemplate, render } from "./desks.mjs";
...
// desk file — a desk file is written through a tool. The
...
//
// Beside the rules, one hook: before a Bash call is asked about, lib/hooks/compound.mjs lets it
// through when every side of it is a command these rules already hold. Mechanism, not a rule — it
// grants nothing the list above does not — so an update wires it into settings that lack it
// (`wireHook`), where it adds no rule.
//
...
function settingsToStartWith(leader) {
  return {
    permissions: {
      allow: [...TOOL_RULES, ...READ_RULES, ...TREE_RULES, ...SHELL_RULES, ...deskRules(leader)],
      deny: [...OWN_ACCOUNT_RULES, ...SHELL_DENY_RULES, ...DESK_DENY_RULES],
    },
...
// The hook into settings that have not got it, the rest of the file left as it is. Answers with
// what it wrote: the settings file, or nothing when the hook was there or the file is not ours to
// read (`hookWired(readSettings(root))` tells the two apart).
export function wireHook(root) {
  const settings = settingsReadable(root);
  if (settings === null || hookWired(settings)) {
    return [];
  }
  const hooks = typeof settings.hooks === "object" && settings.hooks !== null ? settings.hooks : {};
  const before = Array.isArray(hooks.PreToolUse) ? hooks.PreToolUse : [];
  return writeSettings(root, { ...settings, hooks: { ...hooks, PreToolUse: [...before, HOOK_ENTRY] } });
}
...
// Answers the way `wireHook` does: the settings file it wrote, or nothing when the file already
...
// The rules a fresh instance is born with that these settings have not got, allow and deny alike,
// each spelt as it would stand in the file. An update says them and adds none: what a workspace
// allows is its owner's, and a rule they took out is a rule they took out.
export function rulesMissing(root) {
  ...
  return [
    ...[...TOOL_RULES, ...READ_RULES, ...TREE_RULES, ...SHELL_RULES].filter((rule) => !allow.includes(rule)).map((rule) => `allow ${rule}`),
    ...[...OWN_ACCOUNT_RULES, ...SHELL_DENY_RULES, ...DESK_DENY_RULES].filter((rule) => !deny.includes(rule)).map((rule) => `deny ${rule}`),
  ];
}
