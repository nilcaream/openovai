...
import { DESK_DENY_RULES, DESK_TEMPLATE, OWN_ACCOUNT_RULES, READ_RULES, TOOL_RULES, TREE_RULES, deskFile, deskRules, readTemplate, render } from "./desks.mjs";
...
// tools the chat serves, read any file in the instance, and write under the three trees the User
// works in; the Leader may write in its own desk directory, the way every person hired later may
...
function settingsToStartWith(leader) {
  return {
    permissions: {
      allow: [...TOOL_RULES, ...READ_RULES, ...TREE_RULES, ...deskRules(leader)],
