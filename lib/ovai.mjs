...
import { rulesMissing, seedUserContent, turnAttributionOff, wireHook } from "./seed.mjs";
import { readSettings, settingsFile } from "./settings.mjs";
import { STORE_DIRECTORY, stamp, zoneOf } from "./store.mjs";
...
// exists without touching it; ESRCH is the answer wanted.
function leftRunning(root, serverPid) {
  ...
  return [...server, ...runningHere(root)];
}
...
async function startServer(root) {
  ...
  // it said is what somebody reading the log afterwards is there for. Each run is headed by one
  // line naming its process and the moment it was started, so a reader knows where one run's
  // account ends and the next begins. The header is written here rather than by the server: a
  // foreground server says one line, its address, and nothing else.
  //
  ...
  const before = fs.fstatSync(log).size;
  // The moment is taken before the child exists: the first stamp in a process loads the locale
  // and zone data, and a child that boots while the parent is still stamping says its address
  // ahead of the header. Only the pid has to wait for the spawn, and the header is written the
  // moment it is known; the offset is where this run's own output begins.
  const started = stamp(new Date(), zoneOf());
  const child = spawn("node", [SERVER, "--root", root], {
    cwd: root,
    detached: true,
    stdio: ["ignore", log, log],
  });
  const header = `--- ovai start: server pid ${child.pid} at ${started}\n`;
  fs.writeSync(log, header);
  const from = before + Buffer.byteLength(header);
  fs.closeSync(log);
  child.unref();
