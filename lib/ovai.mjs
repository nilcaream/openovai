...
import { changedBetween, differences, fingerprint, record as recordAdmin, snapshot } from "./admin.mjs";
import { adminSession, hasCredential, home, login, machineToken, onboarded, serverEnvironment } from "./claude.mjs";
...
import { hooksWired, rulesMissing, rulesStale, seedUserContent, turnAttributionOff, wireHooks } from "./seed.mjs";
...
async function update(root, argv) {
  ...
  // The hooks are mechanism and go into settings that lack them; the rules are the person's, and
  // the ones a fresh instance is born with that these settings have not got are said, not added.
  const wired = wireHooks(root);
  if (wired.length > 0) {
    console.log("Wired the hooks that let a compound of allowed commands through and hand a subagent customization/common.md, into:");
    for (const entry of wired) {
      console.log(`  ${entry}`);
    }
  } else if (!hooksWired(readSettings(root))) {
    console.log(`Left ${settingsFile(root)} as it is, since it could not be read as JSON; the hooks that let a compound of allowed commands through and hand a subagent customization/common.md are not wired.`);
  }
...
async function stopServer(root) {
  ...
  const { park } = settingsIn(readConfig(root));
  const patience = (park.timeout + park.margin) * 1000;
  const began = Date.now();
  const pause = () => new Promise((resolve) => setTimeout(resolve, 200));
  while ((await healthAt(running.url)) !== null) {
    if (Date.now() - began > patience) {
      throw new ServerError(`the server at ${running.url} is still answering after ${patience / 1000} seconds`);
    }
    await pause();
  }
  ...
  while (left.some(({ session }) => session)) {
    if (Date.now() - began > patience) {
      throw new ServerError(`${patience / 1000} seconds after asking the server to stop, ${describeRunning(left)}`);
    }
    sessionsSeen ||= left.some(({ pid: one }) => one !== pid);
    await pause();
    left = leftRunning(root, pid);
  }
  console.log(sessionsSeen ? `Stopped (sessions gone after ${((Date.now() - dark) / 1000).toFixed(1)} seconds).` : "Stopped.");
...
async function startServer(root) {
  ...
  // was started with, and a variable a process sets on itself changes nothing it says. And
  // without the seat or the session it was typed in (serverEnvironment): a server carrying a
  // seat's name is read as that seat's leftover, and one carrying a session's own id and socket
  // hands them to every session it starts.
  ...
  for (;;) {
    ...
    if (Date.now() - began > START_PATIENCE) {
      throw new ServerError(`the server (pid ${child.pid}) has not answered after ${START_PATIENCE / 1000} seconds; its output is in ${where}`);
    }
...
function adminHere(root) {
  ...
  if (!onboarded(root)) {
    console.log("The first time, Claude Code runs its own first-launch setup; this instance is already signed in, so if it asks for an account, pick the same one.");
  }
  console.log("");

  // What the configuration was before the door opened, as names, so what changed behind it can be
  // named without carrying a value of any of it.
  const before = fingerprint(root);
  const opened = snapshot(root);
  const status = adminSession(root, config.auth, briefing);
  // Written whether or not anything changed: that admin mode ran at all is what the Leader is
  // owed. A running server hands it over within seconds, a stopped one at its next start.
  recordAdmin(root, { ended: new Date().toISOString(), changed: changedBetween(before, fingerprint(root)), moved: differences(opened, snapshot(root)) });

  console.log("");
  console.log("Admin mode closed. A seat reads its configuration when it starts, so nothing changed in there is live yet.");
  console.log("The Leader is told now if the server is running, at its next start if not: which marketplaces, plugins, skills, MCP servers, permission rules and settings changed — by name, and nothing of what is in them.");
