...
import { changedBetween, fingerprint, record as recordAdmin } from "./admin.mjs";
...
function adminHere(root) {
  ...
  // What the configuration was before the door opened, so what changed behind it can be named
  // without reading a line of any of it.
  ...
  // owed. The server hands it over at its next start, which is the first moment any of it is live.
  recordAdmin(root, { ended: new Date().toISOString(), changed: changedBetween(before, fingerprint(root)) });

  console.log("");
  console.log("Admin mode closed. A seat reads its configuration when it starts, so nothing changed in there is live yet.");
  console.log("The Leader is told at the next start that admin mode ran, and which of this instance's configuration files changed — by name, and nothing of what is in them.");
