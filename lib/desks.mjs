...
//
// And AskUserQuestion, whose question is itself a card on the panel: without the rule the session
// is first stopped on a card asking whether it may ask. Measured on 2.1.280: the rule does not skip
// the permission callback for this tool, so the question still reaches the panel.
export const TOOL_RULES = ["mcp__openovai", "AskUserQuestion"];
...
//
// The number is a checkpoint, not a cap: whoever writes the brief sizes it for the round, and the
// agent reports when it gets there, done or not. For a Worker the server counts and says so
// (chat/checkpoint.mjs), from the `checkpoint` the Leader sets on its order; for an agent a seat
// starts itself nothing counts, since the server never sees a subagent's calls, and the sentence
// is all there is. Deliberately not built: anything that stops an agent at its number.
...
not a summary of them, not the gist of them, and never left for the agent to work out; the one
thing you change is N, which you replace with the number of tool calls for that round. What is not
...
- Report after N tool calls, done or not: where it stands and what is left; then go on unless told to stop.
...
The first is the checkpoint and the other three are how the work stays small. N is yours to size,
and it differs by round: reading one file is not rewriting three pages. It is a checkpoint and not
a cap: what you get at N is a report and a plan for the rest, and the work goes on. When the Leader
orders a Worker, the number is also given as \`checkpoint\` on that message, and the server counts
the Worker's calls and tells both of them when it is reached. An agent a session starts itself is
counted by nobody — the server never sees its calls — so for that agent the sentence is all there
is. A brief without these is one you are paying for blind.`;
...
export function persona(root, name, { user, leader }) {
  ...
  // The desk goes in as the whole path, the root joined: a session given `desks/<Name>/` has to work
  // out what it is under, and one whose instance sits inside another has been seen to guess the
  // outer one's — the same names, somebody else's desks. Rendered at every start, so a workspace
  // that moved is told where it is now.
  const rendered = render(what, template, { NAME: name, USER: user, LEADER: leader, DESK: deskDirectory(root, name), BUDGET });
