...
// rather than stopping the User for a press that would settle the wrong thing. One rule, the
// exact inverse of the one granted for the desk directory around it (deskRules), and anchored the
// same way, so it refuses from wherever the run is standing. One segment for the name, so it is
// every desk here and nothing beside them.
export const DESK_DENY_RULES = ["Edit(/desks/*/STATE.md)"];
...
//
// The desk file is the exception, and DESK_DENY_RULES is what makes it one: this rule names the
