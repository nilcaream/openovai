...
// is nothing to migrate, and everybody who was never named one moves when it is changed. It is its
...
// look at (`reference/`), what is worked on (`projects/`), and scratch (`temp/`). Both the edit and
// the write rule for each, so a file tool that makes a file and one that changes it are both
// answered by the settings rather than by a press. Relative to the instance root, like every rule
// here, so nothing outside it is granted.
//
...
// The one file no session writes with a file tool: its desk file. STATE.md is written through the
// `write_desk` tool and no other way — that is how the server-owned header and the rule-set
// version get onto line 1 — so a session reaching for it with an editor is refused on the spot
// rather than stopping the User for a press that would settle the wrong thing. Both spellings,
// though one would do: an `Edit(...)` deny on its own refuses the Write tool too (measured, and
// said again where the account files are denied below), so the pair is here to read as the exact
// inverse of the two rules granted for the desk directory around it (deskRules). One segment for
// the name, so it is every desk here and nothing beside them.
export const DESK_DENY_RULES = ["Edit(desks/*/STATE.md)", "Write(desks/*/STATE.md)"];
export const TREE_RULES = USER_TREES.flatMap((tree) => [`Edit(${tree}/**)`, `Write(${tree}/**)`]);

// The shell commands a seat works a repository under `projects/` with, granted when the instance is
// made so that cloning it, branching, staging, committing, and looking at what is about to be
// committed raise no stop; a directory made, and the one `cd` into the repository. Nothing that
// removes or moves a file, nothing that installs, nothing that runs, and no push: the User is the
// one who pushes, and that is kept by the rule's absence, never by a refusal.
//
...
// from the command's first character, so `git -C somewhere add` is not `git add` and asks, and a
// compound asks whatever its halves are (measured: `cd projects/x && git status` stops with every
// rule here in place). The Worker's persona says how to spell it: `cd projects/<repo>` once, alone,
// then plain commands.
...
// session stopped for doing its job. The edit rule and the write rule both, like the trees above,
// so a file made and a file changed are both answered by the settings. One pair per person,
// granted when the desk is opened and withdrawn when it is filed away: a rule for a desk nobody
// has is a grant nobody can account for.
//
// The desk file is the exception, and DESK_DENY_RULES is what makes it one: the pair names the
// whole directory, the deny names the one file, and a deny wins.
export function deskRules(name) {
  return [`Edit(${path.posix.join(DESKS, name)}/**)`, `Write(${path.posix.join(DESKS, name)}/**)`];
}

// Grant the pair, once. Nothing is written when both are already held.
...
// built-in tool that writes a file, the Write tool included, and nothing else.
// A shell command is not one of those tools, and these paths sit inside
