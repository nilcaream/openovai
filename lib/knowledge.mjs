...
const NOT_A_FILE = "not a file: the one directory `knowledge/` holds is `files/`";

// The one directory the shelf holds: the files a note needs of its own, a template or an image, in
// whatever layout and format suits them. None of it is a note, so none of it is read or reported.
const FILES = "files";

// Everything under the directory, in one pass, with the latest mtime it saw. The shelf is notes
// plus `files/`, which is passed over; any other directory is reported the way a stray file is,
// since a session that made one has made something no tool here will ever look in.
function readAll(root) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    if (entry.isDirectory() && entry.name === FILES) {
      continue;
    }
    if (!entry.isFile()) {
