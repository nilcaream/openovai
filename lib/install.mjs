...
// the payload is put in place (lib/release.mjs, the same act an update does) and the person's
// own files are seeded where they are absent (lib/seed.mjs, the same act an update does). An
// install over an instance already in place is therefore an update of it, and neither ever writes over
...
import { CUSTOMIZATION, DESKS, DeskError, KNOWLEDGE, USER_TREES, describeModel, describeName, isModel, isName } from "./desks.mjs";
...
//            browser. Instances still keep their own transcripts.
...
// Every option is required, and the installer never guesses one: a command line that describes
// the whole instance is one that can be read back, repeated and tested.
//
// An answer is looked for in three places, in this order, and the first that has it wins. The
// command line: a person typing `--port 8000` means the port, and it is never asked again. Then,
// over an instance already in place, its openovai.json — not a guess, the instance's own word,
// so an instance being installed over is asked nothing. Then, on a terminal, the person: each
// answer still missing is asked for, one line each, in the order of ASKED, and an answer typed
// is checked exactly as one given on the command line. Off a terminal — a script piping into the
// installer, or reading its output — nothing is asked and what is missing is refused, so that
// nothing is left waiting on a question nobody can see.
const REQUIRED = OPTIONS.map(([, key]) => key);

// What is asked for, in the order it is asked, with the answer Enter gives — or null where
// there is nothing to take for granted: where the instance lives and who is there are the
// person's to say. `--source` is never asked: the `openovai` command passes it, and so does
// the clone example, and the person who has to be asked where the toolkit is has not got it.
const ASKED = [
  ["root", "Where does the instance live", null],
  ["user", "Who does the team work for", null],
  ["leader", "Who leads the team", null],
  ["leaderModel", "Which model does the Leader run on", "opus"],
  ["workerModel", "Which model do Workers run on", "opus"],
  ["port", "Which port does the chat page listen on, 0 for one picked at start", String(PORT_CHOSEN_AT_START)],
  ["auth", `How does the instance sign in, ${AUTH_MODES.join(" or ")}`, "login"],
];
...
//
//   desks/         one directory per person: the desk a replacement session reads, the persona
//                  the session was last started with, the conversation the chat kept for it
//   reference/     what is kept to look at: documents, sources, clones for analysis; never
//                  worked on
//   projects/      what is worked on; a reference that needs edits is cloned fresh here
//   temp/          scratch, deletable by anyone at any time
//   .claude/       settings that belong to the instance and can be shared
//   .local/        the instance's own Claude Code config directory: its account, transcripts
//                  and memory, kept apart so two instances on one machine never share a
//                  session history
//   plugins/       tools this instance serves itself, one file per tool, written where the
//                  machine they reach out to is known
//   customization/ what the person adds to a persona, one file per kind of session; theirs,
//                  never touched by an update
//   knowledge/     what the workspace knows, one Markdown note per topic, written by every seat
//                  with the file tools and read through the index and validate tools
//
...
// into the other. The three the User works in are made here rather than by the first session
// that needs one, because the grants that name them are written here too, and a rule for a
// directory that is not there is a rule somebody has to explain. `customization/` is made here
// for the same reason a person needs a directory to find: a fresh instance shows every place a
// thing of theirs can go, so the layout is read off the root and not off a document. `knowledge/`
// is made and left empty, which is what a workspace that has learnt nothing yet looks like — the
// directory only, never a note in it, because the first note is something a session wrote.
//
// Two files sit beside these at the root and are written here too: `openovai.json`, the
// instance's description of itself, and `instructions.json`, the settings document its sessions
// run under (lib/instructions.mjs), so the root of a fresh instance is the whole layout.
const LAYOUT = [DESKS, ...USER_TREES, ".claude", HOME, PLUGINS, CUSTOMIZATION, KNOWLEDGE];
...
function usage() {
  return [
    ...
    "",
    "Every option is required and none is guessed. Over an instance that is already there, its",
    "openovai.json answers for whatever is left off, and whatever is given is written into it.",
    "On a terminal, whatever is still missing is asked for; off one, it is refused.",
    "",
    "Options:",
...
// How a question reaches the person: the line to show, and what they typed back. There is one
// only when both ends are a terminal — a script piping into the installer, or reading its
// output, gets none and is refused as it always was — and the checks hand in one of their own.
function askOnTerminal() {
  ...
  return async (line) => {
    ...
    try {
      return await terminal.question(line);
    } finally {
      terminal.close();
    }
  };
...
// One answer, asked for until there is one: Enter takes the default where there is one, and
// asks again where there is none.
async function answer(key, ask) {
  const [, question, fallback] = ASKED.find(([name]) => name === key);
  const line = fallback === null ? `${question}: ` : `${question} [${fallback}]: `;
  for (;;) {
    const typed = (await ask(line)).trim();
    if (typed !== "") {
      return typed;
    }
    if (fallback !== null) {
...
// The command line resolved into the whole description, `ask` being how a missing answer is
// asked for, or null where it cannot be.
async function resolvePlan(parsed, ask) {
  ...
  if (parsed.root === undefined) {
    ...
    parsed.root = await answer("root", ask);
  }
  ...
  for (const key of REQUIRED) {
    ...
    if (parsed[key] === undefined && ask !== null && ASKED.some(([name]) => name === key)) {
      parsed[key] = await answer(key, ask);
    }
    if (parsed[key] === undefined) {
  ...
  // The person's own answers, typed on the command line or in reply to a question: what is
  // written into an instance's openovai.json over what it had.
  const given = new Set(REQUIRED.filter((key) => !fromConfig.has(key)));
...
// one is asked of lib/payload.mjs, because taking a newer version asks the same question of the
...
function printPlan(plan) {
  const rows = [
    ...
    ["server port", plan.port === PORT_CHOSEN_AT_START ? "0 — picked when the server starts" : String(plan.port)],
...
function report(plan, written) {
  ...
  console.log(`  ${ovai} start`);
  if (plan.auth === "inherit") {
    console.log("");
    console.log("It signs in with CLAUDE_CODE_OAUTH_TOKEN from the environment you start it in.");
    console.log(`Mint one once with: ${runtimePaths(plan.root).claude} setup-token`);
  }
}

async function main(argv, ask) {
  try {
    ...
    const plan = await resolvePlan(parsed, ask);
    printPlan(plan);
    checkSource(plan);
    checkRoot(plan);
...
export { UsageError, resolvePlan };

// Run as the installer when it is the command; imported, it is the planning the checks drive.
if (process.argv[1] !== undefined && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  process.exitCode = await main(process.argv.slice(2), askOnTerminal());
}
