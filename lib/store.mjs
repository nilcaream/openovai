...
// the files and never a flag kept beside them. The way back is the same shape: a restore is one more
// file, a marker naming the record it brings back, and the walk over the files in order — a
// `supersedes` marks a record replaced, a `restores` unmarks it — is what says which records stand.
// A marker is not a record: it is in no list and no answer but the one that says what it is.
//
...
// a text, an id it was not sent, or the decision to refuse. NOTHING SILENT: the store never chooses
// what a write replaces. A write that names no `replaces` and reads, to the helper or byte for
// byte, as a record already held is refused with that record — its id, its text, the reason — and
// the writer answers by naming it or by saying `replaces: none`.
...
// The one word in `replaces` that says "nothing": the writer has read the store and this is a new
// record, so nothing is asked about it and nothing is refused on a resemblance.
const NONE = "none";
...
// The fields, in the order the file holds them. `text` is the body and not a field. A marker has
// `restores` and no kind; a record never has `restores`.
const FIELDS = ["id", "store", "kind", "source", "scope", "number", "until", "by", "at", "supersedes", "replaced", "reason", "restores"];
...
// THE ONE READ. Currency first, over everything, in file order: a record named in another's
// `supersedes` is replaced, whoever may see either, and a marker's `restores` makes it current again
// — until something later replaces it once more. Then the scope: a Worker is handed nothing scoped
// to the Leader. Everything a caller can be shown, and everything the helper is ever sent, comes out
// of here with that caller's role. Markers are in none of it; `markers` holds them for the one
// answer that names one.
//
// `successor` maps a replaced id to the id that replaced it, so a refusal can name where a record
// went — over the visible records only, so what it names is something the caller may see.
// `restored` maps a record brought back to the marker that did it, and to the id that had replaced
// it, so its line can say so. `next` is the number the next file takes, read off the same listing:
// a write decides it here, under the store's chain, and the "wx" at the open is what catches
// anything that appeared in between.
export function readStore(root, store, { role }) {
  const every = recordsIn(root, store);
  const replaced = new Map();
  const restored = new Map();
  for (const one of every) {
    if (one.supersedes !== undefined) {
      replaced.set(one.supersedes, one.id);
      restored.delete(one.supersedes);
    }
    if (one.restores !== undefined) {
      restored.set(one.restores, { marker: one, from: replaced.get(one.restores) });
      replaced.delete(one.restores);
    }
  }
  const records = every.filter((one) => one.restores === undefined);
  const visible = role === LEADER ? records : records.filter((record) => record.scope !== "leader");
  ...
  return {
    ...
    successor,
    restored,
    markers: every.filter((one) => one.restores !== undefined && seen.has(one.restores)),
    ...
    version: setVersion(records.filter((record) => !replaced.has(record.id))),
...
// who, when, and what it replaced with the replaced text quoted; then, for a record brought back,
// who restored it and when, and what had replaced it — which stands: a restore is not a swap. The
// text is never paraphrased.
export function recordLine(record, { now, zone, successor, restored = new Map() }) {
  ...
  if (restored.has(record.id)) {
    const { marker, from } = restored.get(record.id);
    back = `; restored by ${marker.by}, ${said(marker.at, zone)} (had been replaced by ${from}, which stands)`;
  }
  return `${first}\n      ${who}${replaced}${back}`;
}

// A marker, asked for by its id: it is not a record, and the answer says what it did instead.
function markerLine(marker, zone) {
  return `${marker.id} is not a record: it restored ${marker.restores} (by ${marker.by}, ${said(marker.at, zone)})`;
}
...
export async function recall(root, args, { caller, now = new Date(), config, helper, zone = zoneOf() }) {
  ...
  const { records, current, successor, restored, markers, version } = readStore(root, store, { role: caller.role });
  const line = (record) => recordLine(record, { now, zone, successor, restored });

  if (by === "id") {
    const record = records.find((one) => one.id === args.id);
    if (record === undefined) {
      const marker = markers.find((one) => one.id === args.id);
      return { text: marker === undefined ? `no record ${args.id} in ${store}` : markerLine(marker, zone) };
    }
...
// pass, and nothing is written. Steps 3 to 9 run under the store's chain. `restore: <id>` is the
// other thing this tool does, alone: bring a replaced record back, under the same chain.
export function remember(root, args, context) {
  if (args?.restore !== undefined) {
    const shape = restoreShape(args);
    if (shape.refused !== undefined) {
      return Promise.resolve(shape);
    }
    return serialized(root, shape.store, () => restore(root, shape, context));
  }
...
function rememberShape(args, caller) {
  ...
  if (args.reason !== undefined && (args.replaces === undefined || args.replaces === NONE)) {
    return { refused: "reason goes with replaces naming a record; leave it out or name what this replaces" };
  }
  return { store, kind, source, scope, text, until: args.until, replaces: args.replaces, reason: args.reason };
}

// A restore's arguments: the store and the id, nothing else — a restore brings a record back as it
// was, so there is no text, kind or reason to give.
function restoreShape(args) {
  const store = args.store;
  if (!STORES.includes(store)) {
    return { refused: `store is ${STORES.join(" or ")}` };
  }
  if (typeof args.restore !== "string") {
    return { refused: "restore is a record id such as k47" };
  }
  const beside = Object.keys(args).filter((name) => name !== "store" && name !== "restore");
  if (beside.length > 0) {
    return { refused: `restore goes alone with store; take ${beside.join(", ")} out` };
  }
  return { store, id: args.restore };
}
...
async function write(root, shape, { caller, now = new Date(), config, helper, zone = zoneOf() }) {
  ...
  // Step 4: what this replaces. Named by the writer, or `none`; or — `replaces` left out — looked
  // for: the same text again, else whatever the helper judges to be this record restated, widened,
  // narrowed or reversed. Found, the write is REFUSED with the record found and why: the store never
  // replaces on its own judgement, and the writer answers with `replaces: <that id>` or `none`.
  let replaces = null;
  let reason = shape.reason;
  if (shape.replaces !== undefined && shape.replaces !== NONE) {
    ...
    replaces = named;
    reason = reason ?? TAG;
  } else if (shape.replaces === undefined) {
    const same = sameKind.find((record) => collapse(record.text) === collapse(text));
    if (same !== undefined) {
      return { refused: resembles(same, "the same text") };
    }
    if (sameKind.length > 0) {
      ...
      if (found !== undefined) {
        return { refused: resembles(found, asked.answer.reason) };
      }
    }
  }
  ...
  // Step 10: the answer — the record's line — and the announcement, for a hard rule only.
  const lines = [recordLine(record, { now, zone, successor: new Map() })];
  if (kind === HARD_RULE) {
...
// The refusal of a write that reads as a record already held: that record, and the two ways on.
function resembles(record, why) {
  return `not written: this reads as [${record.id}] "${record.text}" restated, widened, narrowed or reversed — ${why}; re-issue with replaces: ${record.id} if so, or replaces: ${NONE} for a new record`;
}

// Bring a replaced record back, live under its own id, by writing a marker that names it. The
// record that replaced it stands: a restore is not a swap, and a text that was written over it is
// somebody's and stays. Refused: an id this caller cannot see or that is not a record, a record
// that is current, a hard rule (two live rules with one number is a broken set — it comes back by
// being written again over the rule that holds the number), and the User's record from a Worker.
function restore(root, shape, { caller, now = new Date(), zone = zoneOf() }) {
  const { store, id } = shape;
  const { records, current, replaced, next } = readStore(root, store, { role: caller.role });
  const record = records.find((one) => one.id === id);
  if (record === undefined) {
    return { refused: `${id} is not a record of ${store}; restore names a replaced record by its id` };
  }
  if (current.includes(record)) {
    return { refused: `${id} is current; nothing to restore` };
  }
  if (record.kind === HARD_RULE) {
    return { refused: `${id} is a hard rule; a hard rule comes back by being written again with replaces naming the rule that holds number ${record.number}` };
  }
  if (record.source === "user" && caller.role !== LEADER) {
    return { refused: `${id} is the User's ${record.kind}; the Leader restores it` };
  }
  const from = replaced.get(id);
  const marker = {
    id: `${PREFIX[store]}${next}`,
    store,
    by: caller.role === LEADER ? LEADER : `${caller.name} (${WORKER})`,
    at: stamp(now, zone),
    restores: id,
    text: "",
  };
  fs.writeFileSync(path.join(storeDirectory(root, store), fileName(next)), serialise(marker), { flag: "wx" });
  return { text: recordLine(record, { now, zone, successor: new Map(), restored: new Map([[id, { marker, from }]]) }) };
}
