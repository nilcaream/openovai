...
// kept in the instance's own Claude Code config directory (`.local/`), beside the account settings
// that are handed to the same runs — an instance that was moved carries no list for where it used
// to be, and nothing of it lands in the instance root.
//
...
// The settings document a run is started with, inside the instance's Claude Code config directory.
// Named for what it holds rather than as "settings", so that nobody reads it for the instance's
// settings — those are in `.claude/settings.json` and are the person's — nor for Claude Code's
// own, which sit beside it under the name Claude Code gave them.
const FILE = path.join(HOME, "instructions.json");
