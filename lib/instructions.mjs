...
// written at the instance root, beside `openovai.json`: it is the instance's own description of
// what its sessions run under, as `openovai.json` is its description of itself, and not Claude
// Code's — `.local/` holds what Claude Code keeps, the account, the transcripts, the memory. An
// instance that was moved carries no list for where it used to be, because the next start writes
// the list for where it is.
//
...
// The settings document a run is started with, at the instance root. Named for what it holds
// rather than as "settings", so that nobody reads it for the instance's settings — those are in
// `.claude/settings.json` and are the person's — nor for Claude Code's own, which sit under
// `.local/` under the name Claude Code gave them.
...
// on every call — the installer's, so a fresh instance has it, and the chat's at every run — so a
// run is always started with the list for where the instance is now; the file is tiny and the
// write is cheaper than any reasoning about whether it is current.
