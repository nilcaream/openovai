...
//
// It is a file inside the payload rather than a field in openovai.json, and that is what makes
// taking a newer version need no conversion of anything: the version travels with the code it
// names, so replacing the payload replaces it too and the two cannot disagree. There is no field
// to rewrite and nothing to look up. It sits in `lib/` and not at the root because it is a
// property of what the release ships, and `lib/` is where everything the release ships apart from
// the command lives; the root of an instance is the layout and nothing else.
//
...
export const VERSION_FILE = path.join("lib", "VERSION");
...
// real thing to be standing in front of, and `ovai configuration` is the command somebody runs when they
...
// standing in front of — a release made from a directory, or one built where somebody was
