...
export const COMPOUND_REASON = "not run: this line is more than one plain command (a pipe, &&, ;, a loop, an assignment, $( or a here-document) and would stop on a card. Create the script with the Write tool, in temp/ or on your desk, then run only `bash <file>` as its own Bash call. Read a file's lines with Read, not sed or cat.";
...
// One plain command: a single side that opens with its program, not with an assignment, a
// negation or a loop or branch head. Such a line that no rule holds is the User's to answer, on
// the card; anything more is the session's to write as a script.
function plain(sides) {
  const first = sides.length === 1 ? sides[0].split(/\s+/)[0] : "";
  return sides.length === 1 && first !== "for" && !OPENERS.has(first) && !ASSIGNMENT.test(first);
}
