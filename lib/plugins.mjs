...
//
// A file in here is a tool a session chooses to call, and nothing else: no file here reaches the
// person, since a tool that did would hand a Leader a second way to them — the page notifies, and
// the page is not a tool.
//
...
// the directory IS the list, the same way a directory under desks/ is a person. Renaming a tool is
...
// Where an instance keeps them. At the root beside desks/, and deliberately NOT
// under lib/: taking a newer version of the toolkit removes every payload directory before it
...
//
// Not under desks/ either, and for a sharper reason: that directory listing IS the roster, so a
// directory in it is a person. Plugins live where they cannot be mistaken for one.
export const PLUGINS = "plugins";

// What one looks like before a name is written into it. It lives in lib/templates/ beside the desk and
...
export const PLUGIN_TEMPLATE = path.join("lib", "templates", "plugin.mjs");
...
// serves a Leader is exactly it but `done`, and a Worker its own few, so a tool of the chat's own
// that nobody added here is a red suite.
export const BUILT_IN = ["message", "room", "index", "validate", "write_desk", "restart_session", "stop_session", "done", "stop_worker", "restart_worker", "park", "hire", "retire", "permission"];
...
// so a directory read on every call would show a NEW file and go on serving the stale code of a
